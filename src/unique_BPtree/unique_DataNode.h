#ifndef BPTREE_UNIQUE_DATANODE_H
#define BPTREE_UNIQUE_DATANODE_H
#pragma once
...
namespace arima_kana {
    ...
    public:
      ...
      unique_DataNode() = default;

      explicit unique_DataNode(const p &kv) : size(1) {
        _data[0] = kv;
      }
      ...
      void insert_pair(K key, V val) {
        ...
        while (l < r) {
          int mid = (l + r) / 2;
          if (_data[mid].first < key) l = mid + 1;
          else r = mid;
        }
        if (_data[l].first == key) {
      ...
      bool operator==(const unique_DataNode &other) const {
