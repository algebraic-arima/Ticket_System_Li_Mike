#ifndef BPTREE_UNIQUE_BPTREE_H
#define BPTREE_UNIQUE_BPTREE_H
#pragma once
...
//        }
      ...
      void divide_node(size_t pos) {
        ...
        if (node._par == 0) {
          ...
          par_node.insert_pair(new_node._key[new_node._size - 1], new_pos);
      ...
      void insert_max_adjust(const K &kv) {
        size_t pos = root;
      ...
      void subs(size_t pos, const K &old_kv, const K &new_kv) {
        Node &node = list[pos];
...
//        while (i < node._size && node._key[i] < old_kv) ++i;///binary search
      ...
      size_t list_lower_bound(const K &kv) {
        size_t pos = root;
        Node *node = &list[pos];
        while (!node->is_leaf) {
      ...
      void merge(size_t l, size_t r) {
        ...
        list[par].remove_pair(list[l]._key[list[l]._size - 1]);
...
//          error("tree: Key not found");
      ...
      size_t block_lower_bound(const K &k) {
        size_t pos = list_lower_bound(k);
        if (pos == 0) return 0;
        Node &node = list[pos];
        size_t i = node._size;
        while (i > 0 && !(node._key[i - 1] < k)) --i;
        return node._chil[i];
      }

      /// @adjust_max
      /// adjust the maximum pair to kv
      void adjust_max(const K &k) {
        size_t pos = root;
        while (!list[pos].is_leaf) {
          list[pos]._key[list[pos]._size - 1] = k;
          pos = list[pos]._chil[list[pos]._size - 1];
        }
        list[pos]._key[list[pos]._size - 1] = k;
      }
